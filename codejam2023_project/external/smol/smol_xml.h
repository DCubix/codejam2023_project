...
smol_xml_node_vector_t* smol_xml_get_nodes(smol_xml_t* xml);
...
char* smol_xml_node_get_body(smol_xml_node_t* node);
char* smol_xml_node_get_tag(smol_xml_node_t* node);
size_t smol_xml_node_get_id(smol_xml_node_t* node);
...
void smol_xml_find_children(smol_xml_t* xml, smol_xml_node_t* node, const smol_xml_matcher_t* matcher, smol_xml_nodeptr_vector_t* out);
smol_xml_node_t* smol_xml_find_one_child(smol_xml_t* xml, smol_xml_node_t* node, const smol_xml_matcher_t* matcher);

//smol_xml_get_attr - Gets an attribute from a XML node
//Arguments:
// - smol_xml_node_t* node - The XML node
...
const char* smol_xml_get_attr(smol_xml_node_t* node, const char* attr_name);
...
extern size_t _smol_xml_global_id;

#ifdef SMOL_XML_IMPLEMENTATION

size_t _smol_xml_global_id = 0;
...
#define push_node() smol_vector_push(out, node)
	...
	while (!smol_scanner_has_ahead(scan, "</")) {
		// children
		size_t new_id = _smol_xml_global_id++;
...
int _smol_xml_compare_ids(const void* a, const void* b, void* user_data) {
	smol_xml_node_t* na = (smol_xml_node_t*)a;
	smol_xml_node_t* nb = (smol_xml_node_t*)b;
...
smol_xml_node_t* smol_xml_find_one(smol_xml_t* xml, const smol_xml_matcher_t* matcher) {
	...
	smol_vector_each(&xml->nodes, smol_xml_node_t, node) {
		...
		if (!matcher->has_attr || smol_vector_count(&node->attributes) == 0) { // no need to find attributes then
			return node;
		}

		smol_vector_each(&node->attributes, smol_xml_attr_t, attr) {
			int has_attr = matcher->has_attr ? (strcmp(attr->name, matcher->has_attr) == 0) : 1;
			int matched = matcher->attr_equals ? (strcmp(attr->value, matcher->attr_equals) == 0) : 1;
...
void smol_xml_find_children(smol_xml_t* xml, smol_xml_node_t* node, const smol_xml_matcher_t* matcher, smol_xml_nodeptr_vector_t* out) {
	smol_xml_nodeptr_vector_t children;
	smol_vector_init(&children, 1024);
	smol_xml_node_get_children(xml, node, &children);

	smol_vector_each(&children, smol_xml_node_t*, childptr) {
		smol_xml_node_t* child = *childptr;
		int has_tag = matcher->has_tag ? (strcmp(child->tag, matcher->has_tag) == 0) : 1;
		if (!has_tag) continue;

		if (smol_vector_count(&child->attributes) == 0) { // no need to find attributes then
			smol_vector_push(out, child);
			continue;
		}

		smol_vector_each(&child->attributes, smol_xml_attr_t, attr) {
			int has_attr = matcher->has_attr ? (strcmp(attr->name, matcher->has_attr) == 0) : 1;
			int matched = matcher->attr_equals ? (strcmp(attr->value, matcher->attr_equals) == 0) : 1;
			if (!has_attr) continue;
			if (!matched) continue;
