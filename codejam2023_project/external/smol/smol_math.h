...
void smol_m4_decompose(smol_m4_t m, smol_v3_t* position, smol_v3_t* scale, smol_quat_t* rotation);
...
#pragma region smol_v4_t 
 ...
 smol_v4_t smol_v4_from_v3(smol_v3_t v, float w) {
...
smol_quat_t smol_quat_conjugate(smol_quat_t quat) {
	smol_quat_t res;
	res.x = -quat.x;
	res.y = -quat.y;
	res.z = -quat.z;
...
smol_quat_t smol_quat_from_axis_angle(smol_v3_t axis, float angle) {
...
smol_m4_t smol_m4_mul_by_val(smol_m4_t a, float f) {
...
smol_m4_t smol_m4_rotate_axis_angle(smol_v3_t axis, float rotation_rad) {
...
smol_m4_t smol_m4_rotate_quat(smol_quat_t q) {
	smol_m4_t res = smol_m4_identity();
...
void _smol_m4_to_quat(smol_m4_t m, smol_quat_t* q) {
	smol_v4_t* rows = m.rows;
#define a(c, r) rows[r].v[c]
...
void smol_m4_decompose(smol_m4_t m, smol_v3_t* position, smol_v3_t* scale, smol_quat_t* rotation) {
	// axis vectors
	smol_v3_t x = smol_v3(m.m[0], m.m[1], m.m[2]);
	smol_v3_t y = smol_v3(m.m[4], m.m[5], m.m[6]);
	smol_v3_t z = smol_v3(m.m[8], m.m[9], m.m[10]);
	...
	if (scale) {
		(*scale).x = smol_v3_len(x);
		(*scale).y = smol_v3_len(y);
		(*scale).z = smol_v3_len(z);
	}

	smol_v3_t scl = scale ? *scale : smol_v3(1.0f, 1.0f, 1.0f);
